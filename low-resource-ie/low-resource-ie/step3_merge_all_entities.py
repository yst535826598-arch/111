...
def step1_load_entitys_embedding(load_entitys_embedding,deposits_path,final_path,date):
    ...
    if load_entitys_embedding:
        ...
        #full_word_table_addition=eval(str(deposit_history['full_word_table_addition']))
        #full_word_table_label=eval(str(deposit_history['full_word_table_label']))
    ...
    for t_deposit in deposits:
            ...
            for tlabel in deposit_dict.keys():
                ...
                for entitys in deposit_dict[tlabel].keys():
                    if full_word_table_entitys.get(entitys,-100)==-100:
                        ...
                        #emb=temp_word_table_entity[entitys]['嵌入向量']
        ...
        #deposit_history['full_word_table_addition']=full_word_table_addition
        #deposit_history['full_word_table_label']=full_word_table_label
...
def step2_merge_all(load_merge_history,recover,full_word_table_entitys,final_path,date):
    if load_merge_history is False:
        ...
        for word in word_table_entity_mirror.keys():#以后加个已完成百分比
            ...
            emb=word_table_entity_mirror[word]['嵌入向量']
            #similar,similarID=determine_attribute(collection_entitys,emb,6)
...
def step3_load2neo4j(deposit_name,deposit_dict,graph_label_entitys,graph_label_attribute,final_entitys,final_tri):
    ...
    for attr_label in attribute_dict.keys():
        for item in attribute_dict[attr_label]:#figure_dict={'内容':name,'来源':deposit_name+'#'+files+'#'+i}
            t=item['内容'].split('#')
            try:
                entity_data=t[1]
                message=t[0]
    ...
    for label in deposit_dict.keys():
        ...
        for entitys in deposit_dict[label].keys():
            #print(hebing_dict[label][entitys])
            data={}
            deposit_dict[label][entitys] is None
            #for attrubute in hebing_dict[label][entitys].keys():
            #    if attrubute=='内容':
            #        continue
            #    try:
            #        data[attrubute]=hebing_dict[label][entitys][attrubute]['内容']
            #    except:
            #        print("空属性")
            print(entitys+':')
            ...
            #head_ontology=entity_detial['标签']
            ...
            #final_tri=if_relation_exist_plus(head_node,a_node,'实例同义词',final_tri,data=deposit_name)
    ...
    #final_word_array_entity=hebing_dict['实例词表']
    #final_word_array_addition=hebing_dict['扩展词表']#得用总表
    #final_word_array_label=hebing_dict['词类型嵌入表']
    for tri in tri_array:
        ...
        article=tri['article']
        data=article+'#'+textID
        #create_node_plus(graph_label_ontology,entitys)
        #create_node_plus(graph_label_entitys,entitys)
...
if __name__ == '__main__':
    ...
    global_distance=0.6
    load_entitys_embedding=True#假如step1_load_entitys_embedding以后步骤出问题，改True恢复
